...
def fc_layer(in_tensor, hidden_size):
    W = weight_variable([in_tensor.get_shape().as_list()[1], hidden_size])
    b = weight_variable([hidden_size])
...
def buildnet(x):
    ...
    h1 = tf.nn.relu(fc_layer(x, 512))
    h1_drop = tf.nn.dropout(h1, keep_prob)

    h2 = tf.nn.relu(fc_layer(h1_drop, 128))
    W_3 = weight_variable([128, 10])
    b_3 = weight_variable([10])
